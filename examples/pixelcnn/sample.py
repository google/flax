...
def generate_sample(config: ml_collections.ConfigDict, workdir: str):
    ...
    params = train.model(config).init(
        {"params": model_rng, "dropout": dropout_rng}, init_batch
    )["params"]
    ...
    while jnp.any(sample != sample_prev):
        sample_prev, sample = sample, sample_iteration(
            config, sample_rng, params, sample
        )
...
def sample_iteration(config, rng, params, sample):
    ...
    rng, dropout_rng = random.split(rng)
    out = train.model(config).apply(
        {"params": params}, sample, rngs={"dropout": dropout_rng}
    )
