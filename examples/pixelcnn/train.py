...
def train_step(
    ...
    def loss_fn(params):
        ...
        pcnn_out = model(config, dropout_p=config.dropout_rate).apply(
    ...
    ema = jax.tree_multimap(
        lambda ema, p: ema * ema_decay + (1 - ema_decay) * p, ema, optimizer.target
    )
...
def eval_step(config, params, batch):
    images = batch["image"]
    pcnn_out = model(config, dropout_p=0.0).apply({"params": params}, images)
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    initial_variables = model(config).init(
        {"params": init_rng, "dropout": dropout_rng}, init_batch
    )["params"]
    ...
    p_train_step = jax.pmap(
        functools.partial(train_step, config, learning_rate_fn), axis_name="batch"
    )
