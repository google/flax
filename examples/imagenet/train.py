...
def cross_entropy_loss(logits, labels):
    return (
        -jnp.sum(common_utils.onehot(labels, num_classes=1000) * logits) / labels.size
    )
...
def create_train_state(rng, config: ml_collections.ConfigDict, model, image_size):
    ...
    optimizer = optim.Momentum(beta=config.momentum, nesterov=True).create(params)
    state = TrainState(
        step=0,
        optimizer=optimizer,
