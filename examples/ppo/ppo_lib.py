...
def train_step(
    ...
    for batch in zip(*trajectories):
        ...
        l, grad = grad_fn(
            optimizer.target, module, batch, clip_param, vf_coeff, entropy_coeff
        )
        loss += l
...
def process_experience(
    ...
    trajectories = tuple(
        map(lambda x: onp.reshape(x, (trajectory_len,) + x.shape[2:]), trajectories)
    )
    return trajectories
...
def train(
    ...
    for s in range(loop_steps):
        ...
        # Core training code.
        alpha = 1.0 - s / loop_steps if config.decaying_lr_and_clip_param else 1.0
        ...
        for e in range(config.num_epochs):
            permutation = onp.random.permutation(config.num_agents * config.actor_steps)
