...
class RunPretrainingConfig:
  """Configuration for pre-training BERT."""
  # The output directory where the model checkpoints will be written.
...
RunPretrainingConfig.configurable = gin.external_configurable(
    RunPretrainingConfig, name='RunPretraining')
...
def get_config():
  ...
  return RunPretrainingConfig.configurable()
...
def main(argv):
  ...
  if config.do_train:
    train_iter = data.c4_masked_lm_inputs(
  ...
  if config.do_eval:
    eval_iter = data.c4_masked_lm_inputs(
