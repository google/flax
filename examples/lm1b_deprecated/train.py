...
flags.DEFINE_string(
    "data_dir", default=None, help="Directory containing TFDS lm1b/subwords32k dataset."
)
...
flags.DEFINE_integer(
    "max_target_length", default=512, help="Maximum length of training examples."
)

flags.DEFINE_integer(
    "max_eval_target_length", default=2048, help="Maximum length of eval examples."
)
...
flags.DEFINE_string(
    "prompt", default="I love to ", help="Prompt for language model sampling."
)
...
def predict_step(
    ...
    sampled_seqs = decode.temperature_sample(
        ...
        topk=sampling_top_k,
        eos_token=2 ** 16,
    )  # No EOS tokens used in default lm1b dataset encoding.
...
def train_and_evaluate(
    ...
    if jax.host_id() == 0:
        ...
        eval_summary_writer = tensorboard.SummaryWriter(os.path.join(model_dir, "eval"))
    ...
    for step, batch in zip(range(start_step, num_train_steps), train_iter):
        ...
        # Save a Checkpoint
        if (step % checkpoint_freq == 0 and step > 0) or step == num_train_steps - 1:
...
def main(argv):
    ...
    train_and_evaluate(
        ...
        sampling_top_k=FLAGS.sampling_top_k,
