...
def create_learning_rate_schedule(learning_rate: float, warmup_steps: int):
    ...
    return optax.join_schedules(
        [
            ...
            optax.linear_schedule(init_value=learning_rate, end_value=learning_rate / 10, transition_steps=500_000),
        ],
...
def train_step(
    state, batch, dropout_rng, config, learning_rate_fn, label_smoothing=0.0,
):
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    if USE_PMAP or USE_ALPA:
      n_devices = jax.local_device_count()
    else:
      n_devices = 1
    train_ds, eval_ds, _, encoder = input_pipeline.get_datasets(
        n_devices=n_devices, config=config, vocab_path=vocab_path
    )
    ...
    if USE_PMAP:
        ...
        p_eval_step = functools.partial(eval_step, config=eval_config)

        p_pred_step = functools.partial(
                predict_step,
                config=predict_config,
                temperature=config.sampling_temperature,
                top_k=config.sampling_top_k,
