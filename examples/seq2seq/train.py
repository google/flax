...
flags.DEFINE_integer(
    'num_train_steps', default=1000000, help=('Number of train steps.'))
...
def np_one_hot(tokens, vocab_size):
  vecs = np.zeros((tokens.size, vocab_size), dtype=np.float32)
...
def encode_onehot(batch_inputs, max_len=None):
  ...
  def encode_str(s):
    ...
    return np_one_hot(tokens, CTABLE.vocab_size)
...
def get_examples(num_examples):
  ...
  for _ in range(num_examples):
    ...
    key = tuple(sorted((random.randint(0, 9999999), random.randint(0, max_digit))))
...
def train_step(state, batch_query, batch_answer, rng):
  """Train one step."""
...
def train_model(workdir):
  ...
  logging.info(f'Number of devices found: {num_devices}')

  # create optimizer and params
  model = Seq2seq(teacher_force=False, hidden_size=FLAGS.hidden_size)
  ...
  rng = jax.random.split(key, num=num_devices)

  for step in range(FLAGS.num_train_steps):
    batch = get_batch(FLAGS.batch_size)  # FLAGS.batch_size is global
    ...
    # training step
    state, device_metrics, rng = p_train_step(state, batch_query, batch_answer, rng)

    # get metrics
    device_metrics = jax.tree_map(lambda x: x[0], device_metrics)
    metrics = jax.device_get(device_metrics)
    loss_val = metrics['loss'].item()
    acc_val = metrics['accuracy'].item()

    if step % 100 == 0:
      logging.info('STEP: {:6}\tLOSS: {:.4f}\tACC: {:.4f}'.format(step, loss_val, acc_val))

    # sampling step
    if FLAGS.sample and step % FLAGS.decode_frequency == 0:
      writer.write_scalars(step, metrics)
      key, lstm_key = jax.random.split(key)
      batch = get_batch(5)
      unrepl_state = jax_utils.unreplicate(state)
      decode_batch(unrepl_state.params, batch, lstm_key)
