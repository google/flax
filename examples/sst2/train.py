...
def compute_metrics(*, labels: Array, logits: Array) -> Metrics:
  ...
  return Metrics(
      loss=jnp.sum(loss), accuracy=jnp.sum(binary_accuracy), count=logits.shape[0]
  )
...
def train_step(
  ...
  def loss_fn(params):
    ...
    logits = state.apply_fn(
        variables, batch['token_ids'], batch['length'], deterministic=False, rngs=rngs
    )

    labels = batch['label']
    if labels.ndim == 1:
      labels = jnp.expand_dims(labels, 1)
    loss = jnp.mean(sigmoid_cross_entropy_with_logits(labels=labels, logits=logits))
    return loss, logits
...
def eval_step(
  ...
  logits = state.apply_fn(
      variables, batch['token_ids'], batch['length'], deterministic=True, rngs=rngs
  )
  metrics = compute_metrics(labels=batch['label'], logits=logits)
  return metrics
...
def normalize_batch_metrics(batch_metrics: Sequence[Metrics]) -> Metrics:
  ...
  return Metrics(loss=total_loss.item() / total, accuracy=total_accuracy.item() / total)
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str) -> TrainState:
  ...
  """
  # Prepare datasets.
  train_dataset = input_pipeline.TextDataset(tfds_name='glue/sst2', split='train')
  eval_dataset = input_pipeline.TextDataset(tfds_name='glue/sst2', split='validation')
  ...
  for epoch in range(1, config.num_epochs + 1):
    ...
    state, train_metrics = train_epoch(train_step_fn, state, train_batches, epoch, rngs)
