...
NOTES:
  - Not setup for dynamic batching with multihost training!
...
def bin_and_batch(dataset,
  ...
  if buckets is None:
    ...
    if not training:
      ...
      bucket_batch_sizes[-1] = batch_size // max_eval_length
...
def get_wmt_datasets(n_devices,
  ...
  keys = ([features_info.supervised_keys[0]], [features_info.supervised_keys[1]])

  # TFDS's supervised keys looks like: (('en',), ('de',),)
  # we want to map data to 'inputs' and 'targets' features.
  # e.g. {'en': example_in, 'de': example_out} to
  #      {'inputs': example_in, 'targets': example_out}
  to_features_dict = (
      lambda x: {'inputs': x[keys[0][0]], 'targets': x[keys[1][0]]})
  ...
  def encode(lang1, lang2):
    res1 = [subword_tokenizer.vocab_size] + subword_tokenizer.encode(
        lang1.numpy()) + [subword_tokenizer.vocab_size+1]
    res2 = [subword_tokenizer.vocab_size] + subword_tokenizer.encode(
        lang2.numpy()) + [subword_tokenizer.vocab_size+1]
    return res1, res2
