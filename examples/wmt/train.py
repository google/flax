...
def train_step(
    optimizer, batch, config, learning_rate_fn, label_smoothing=0.0, dropout_rng=None
):
...
def evaluate(*, p_eval_step, target, eval_ds: tf.data.Dataset, num_eval_steps: int):
    ...
    *,
    ...
    for pred_batch in predict_ds:
        ...
        predicted = p_pred_step(
            pred_batch["inputs"], target, cache, decode.EOS_ID, max_predict_length
        )
