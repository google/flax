...
# Finite temperature sampling is a reasonably simple way to
# sample from language models that offers decent results, it
# is also considerably simpler than full beam search and is
# a useful minimal example of an autoregressive sampler.
...
def temperature_sample(inputs,
  ...
  i0 = jnp.array(0)
  token0 = jnp.zeros((batch_size, 1)).astype(jnp.int32)
  ended0 = jnp.zeros((batch_size, 1)).astype(jnp.bool_)
  sequences0 = jnp.zeros((batch_size, max_decode_len)).astype(jnp.int32)
  # Sampling loop state is stored in a simple tuple.
  sampling_loop_init_state = (i0, sequences0, init_cache,
                              token0, ended0, init_rng)
