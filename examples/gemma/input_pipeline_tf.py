...
def get_raw_dataset(dataset_name: str, split: str) -> tf.data.Dataset:
  ...
  ds = tfds.load(dataset_name, split=per_host_split)
...
def pack_dataset(
  ...
  dataset = dataset.padded_batch(
    ...
    padding_values={k: pad_id for k in keys},
  )
...
def shift_target_left(x: Features, pad_id: int) -> Features:
  ...
  targets = x["targets"][..., 1:]
  targets = tf.pad(targets, [(0, 0), (0, 1)], "constant", constant_values=pad_id)
  x["targets"] = targets
  return x


def move_to_devices(
  x: Features, data_sharding: jax.sharding.Sharding
) -> dict[str, jax.Array]:
  return jax.tree.map(
...
def preprocess_data(
  ...
  if shift:
    dataset = dataset.map(
      lambda x: shift_target_left(x, pad_id=pad_id),
      num_parallel_calls=AUTOTUNE,
      deterministic=True
    )
  ...
  if data_sharding is not None:
    dataset = map(lambda x: move_to_devices(x, data_sharding=data_sharding), dataset)
  else:
    dataset = map(lambda x: jax.tree.map(jax.numpy.asarray, x), dataset)
...
def get_datasets(
  ...
  *,
  ...
  sp_processor = tokenizer.load_or_train_tokenizer(
    ...
    tokenizer_type="tftxt",
  )

  sp_processor.pad_id = lambda : pad_id

  train_data = train_data.map(
    tokenizer.TokenizeOpTFTensor(sp_processor), num_parallel_calls=AUTOTUNE
  )
