...
class SamplerTest(parameterized.TestCase):
  ...
  def test_samples(self):
    ...
    with jax.set_mesh(self.mesh):
      ...
      result = sampler(
          ['input string', 'hello world'], total_generation_steps=10
      )
  ...
  def test_forbidden_tokens(self):
    ...
    with jax.set_mesh(self.mesh):
      ...
      transformer.embedder.embedding.set_value(
          jnp.eye(vocab.GetPieceSize(), 32)
      )
      sampler = sampler_lib.Sampler(
          transformer=transformer,
          vocab=vocab,
  ...
  def test_forward_equivalence(self):
    ...
    with jax.set_mesh(self.mesh):
      ...
      out_logits = np.array(output_transformer.logits)[
          0, 1 : n_input_tokens + 1
      ]
