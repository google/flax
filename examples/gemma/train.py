...
def jax_train_step(
  ...
  train_metrics.update(
      loss=loss_per_sample, logits=logits, labels=targets, mask={"accuracy": input_mask}
  )
...
def train_and_evaluate(config: TrainConfig, workdir: str, chpt_bucket: str | None = None):
  ...
  shd_config = transformer_lib.ShardingConfig.fsdp_tp_sharding(
  ...
  jit_train_step = jit_fn(
      nnx_train_step if config.use_nnx_transforms in ("all", "grad-only") else jax_train_step,
      static_argnames=("label_smoothing", "pad_id"),
      donate_argnames=("model", "optimizer"),
  )
  jit_eval_step = jit_fn(
      nnx_eval_step if config.use_nnx_transforms in ("all", "grad-only") else jax_eval_step,
      static_argnames=("label_smoothing", "pad_id"),
  )
    ...
    for step in range(start_step, config.num_train_steps):
      ...
      with maybe_profiler_step_trace:
        ...
        with report_progress.timed('train_step'):
          ...
          if output is not None:
            model, optimizer, rngs, train_metrics = output
