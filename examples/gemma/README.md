...
export TPU_NAME=$USER-flax-gemma-lm1b-${ACCELERATOR_TYPE}
...
```bash
gcloud compute tpus tpu-vm ssh --worker=all --command="journalctl -u google-startup-scripts.service | tail -n 2" $TPU_NAME
...
```bash
gcloud compute tpus tpu-vm ssh $TPU_NAME --worker=all --command="ps -aux | grep python"
```
