...
def shift_target_left(
    x: dict[str, np.ndarray], pad_id: int
) -> dict[str, np.ndarray]:
...
def preprocess_data(
  ...
  if shift:
    ...
    # pad_id, bos_id, eos_id = 0, 2, 1
    #  inputs:  2  t1 t2 t3 1 | 2  T1 T2 1 | 0  0
    # targets:  t1 t2 t3 1  2 | T1 T2 1  0 | 0  0
  ...
  if data_sharding is not None:
    dataset = dataset.map(
        lambda x: move_to_devices(x, data_sharding=data_sharding)
    )
  else:
...
def get_datasets(
  ...
  train_data = train_data.map(tokenizer.TokenizeOpNumpy(sp_processor))
  eval_data = eval_data.map(tokenizer.TokenizeOpNumpy(sp_processor))
