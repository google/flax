...
class Sampler:
  ...
  def init_sample_state(
    ...
    token_buffer = jnp.full(
        (batch_size, buffer_size), self.vocab.pad_id(), dtype=jnp.int32
    )
    ...
    if data_sharding is not None:
      ...
      num_input_tokens = jax.device_put(num_input_tokens, data_sharding)
  ...
  def __call__(
    ...
    masked_token_buffer = np.asarray(masked_token_buffer)
    ...
    num_input_tokens = np.asarray(num_input_tokens)
