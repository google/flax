...
def _resolve_bound_callable(
  ...
  """Resolves bound methods to unbound callables for transform compatibility.

  Args:
    f: A callable that may be a bound method of an NNX Module.
  ...
  """
  from flax.nnx.module import Module  # local import to avoid cycles
...
def _shift_indices(
  indices: int | tuple[int, ...] | None, offset: int
) -> int | tuple[int, ...] | None:
  """Shift positional index/indices by the given offset.

  Args:
    indices: Index specification (int, tuple of ints, or None).
    offset: Offset to add to each index (e.g., +1 when injecting Module as arg0).

  Returns:
    Shifted indices with the same structure as input.
  """
  if indices is None:
    return None
...
def _prepend_in_axes(in_axes: tp.Any, module_axes: tp.Any = None) -> tp.Any:
  """Prepend a module axis to in_axes for bound method auto-unbinding.

  Args:
    in_axes: Original in_axes specification.
    module_axes: Axis specification for the injected Module argument
      (default None means broadcast).

  Returns:
    Modified in_axes with module_axes prepended.
  """
  # If already a tuple/list, prepend; otherwise return a two-tuple.
  if isinstance(in_axes, tuple):
    return (module_axes,) + in_axes
  if isinstance(in_axes, list):
    return [module_axes] + list(in_axes)
  return (module_axes, in_axes)
...
def eval_shape(
  ...
  **kwargs: tp.Any,
  ...
  f_call, bound_self, was_bound = _resolve_bound_callable(f)

  if was_bound:
    args, kwargs = extract.to_tree(((bound_self, *args), kwargs))
  else:
