...
def set_transform_axis_name(f: tp.Callable | None = None,
                            name: str | None = None, index: int = 0):
  if f is None:
    return functools.partial(set_transform_axis_name, name=name, index=index)
  @functools.wraps(f)
  def wrapper(*args, **kwargs):
    args, kwargs = remove_axis((args, kwargs), index, {PARTITION_NAME: name})
    out = f(*args, **kwargs)
    out = add_axis(out, index, {PARTITION_NAME: name})
    return out
