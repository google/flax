...
class VariableContext(threading.local):
  ...
  variable_ref_stack: list[bool] = dataclasses.field(
...
class AbstractVariable(tp.Generic[A], hijax.MutableHiType):
  ...
  _check_can_update = jax_core.aval_method(HijaxVariable._check_can_update)
  ...
  replace = jax_core.aval_method(HijaxVariable.replace)
  to_state = jax_core.aval_method(HijaxVariable.to_state)
...
class Variable(tp.Generic[A], reprlib.Representable, metaclass=VariableMeta):
  ...
  def set_metadata(self, *args, **kwargs) -> None:
    ...
    if len(args) == 1:
      ...
      if 'has_ref' not in metadata:
        raise ValueError('metadata is missing required key `has_ref` key')
      ...
      if 'is_mutable' not in metadata:
        raise ValueError('metadata is missing required key `is_mutable` key')
  ...
  def copy(
    ...
    **updates,
    ...
    if 'is_mutable' in updates and not updates['is_mutable']:
      if 'has_ref' in updates and updates['has_ref']:
        raise ValueError(
          'Cannot set has_ref=True and is_mutable=False simultaneously.'
        )
      if 'is_hijax' in updates and updates['is_hijax']:
        raise ValueError(
    ...
    if ('is_mutable' in updates and updates['is_mutable']) or (
      'has_ref' in updates and updates['has_ref']
    ):
      new_metadata.pop('had_ref', None)
    if ('is_mutable' in updates and updates['is_mutable']) or (
      'is_hijax' in updates and updates['is_hijax']
    ):
