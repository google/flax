...
def current_variable_mode() -> tp.Literal['lojax', 'hijax', 'ref']:
  ...
  mode = config.flax_variable_mode
  if mode not in ('lojax', 'hijax', 'ref'):
    raise ValueError(
      f'Found invalid value for config.flax_variable_mode: {mode!r}. '
      f"Expected one of: 'lojax', 'hijax', 'ref'"
    )
  return mode
...
def variable_mode(value: tp.Literal['lojax', 'hijax', 'ref'], /):
  ...
  return VariableModeContext(prev_value, value)


class VariableModeContext:
  def __init__(
    self,
...
class SetVariable(hijax.HiPrimitive):
  ...
  def jvp(_, primals, tangents, *, treedef, var_type):
    ...
    if type(variable_dot._raw_value) is ad_util.Zero:
      raise Exception(
...
class HijaxVariable(
  ...
  mode = _as_hijax_property('mode', get=True, set=False)
...
class AbstractVariable(tp.Generic[A], hijax.MutableHiType):
  ...
  mode = _as_aval_property(HijaxVariable.mode)
  ...
  get_raw_value = jax_core.aval_method(HijaxVariable.get_raw_value)
...
class VariableMeta(type):
  ...
  def _variable_meta_call(
    ...
    **kwargs,
    ...
    variable = super().__call__(*args, mode=mode, **kwargs)
    if mode == 'hijax':
...
class Variable(tp.Generic[A], reprlib.Representable, metaclass=VariableMeta):
  ...
  def mode(self) -> tp.Literal['lojax', 'hijax', 'ref']:
    return self._var_metadata['mode']
  ...
  def __init__(
    ...
    **metadata: tp.Any,
    ...
    self._raw_value = value  # type: ignore
  ...
  def replace(self, value: tp.Any = Missing, **kwargs) -> Variable[tp.Any]:
    ...
    assert 'raw_value' not in kwargs
  ...
  def from_metadata(
    ...
    if mode == 'hijax':
      return _new_hijax_from_variable(variable)  # type: ignore[assignment]
    elif mode == 'ref':
      if any(not is_array_ref(v) for v in jax.tree.leaves(value)):
        raise ValueError(
          'Got non-Ref value when constructing Variable from metadata with '
          'mode=ref.'
        )
      return variable
    elif mode == 'lojax':
  ...
  def __setstate__(self, state):
    ...
    self._var_metadata = state['_var_metadata']
