...
class RNN(Module):
  ...
  def __call__(
    ...
    *,
    ...
    rngs: rnglib.Rngs | jax.Array | bool = True,
  ):
    ...
    self.rngs: rnglib.Rngs | None
    if rngs is True:
      self.rngs = rnglib.Rngs(0)
    elif rngs is False:
      self.rngs = None
    elif isinstance(rngs, jax.Array):
      self.rngs = rnglib.Rngs(carry=rngs)
    elif isinstance(rngs, rnglib.Rngs):
      self.rngs = rngs
    else:
      raise TypeError(
        f'rngs must be a Rngs, jax.Array, or bool, but got {type(rngs)}.'
      )
