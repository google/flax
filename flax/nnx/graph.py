...
def vars_as(
  ...
  *,
  ...
  """Converts a structure of Variable to 'ref' mode.
  ...
  Example::
    ...
    >>> assert node[0].mode == 'ref'
    >>> assert node[1].mode == 'ref'
  ...
  that can be used to specify which arrays to convert to array refs.
    ...
    ...
    >>> assert mutable_node[0].mode == 'ref'
    >>> assert mutable_node[1].mode == 'pytree'
...
def as_hijax_vars(
  ...
  """ """
  return vars_as(node, is_hijax=True, is_mutable=mutable, only=only)
