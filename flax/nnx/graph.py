...
def find_duplicates(node: tp.Any, /, *, only: filterlib.Filter = ...) -> list[list[PathParts]]:
  ...
  _node_paths(node, node_paths, (), only)
...
def _node_paths(
  ...
  node_paths: dict[int, list[PathParts]],
  path_parts: PathParts,
  duplicate_filter: filterlib.Filter,
  /,
):
  ...
  if _is_graph_node or _is_pytree_node or _is_node_leaf:
    ...
    if node_id in node_paths:
      duplicate_canditate = filterlib.to_predicate(duplicate_filter)
      if (_is_graph_node or _is_node_leaf) and duplicate_canditate(path_parts, node):
        node_paths[node_id].append(path_parts)
      return
    if _is_graph_node or _is_node_leaf:
      node_paths[node_id] = [path_parts]
    node_impl = get_node_impl(node)
    if node_impl is None:
      return
    node_dict = node_impl.node_dict(node)
    for key, value in node_dict.items():
      _node_paths(value, node_paths, (*path_parts, key), duplicate_filter)
