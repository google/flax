...
def is_node_leaf(x: tp.Any) -> tpe.TypeGuard[LeafType]:
  return isinstance(x, LeafType) or variablelib.is_mutable_array(x)
...
def _graph_flatten(
  ...
  if is_variable:
    ...
    mutable_arraydef, leaf = make_mutable_arraydef(node)
...
@tp.overload
@contextlib.contextmanager
def merge_context() -> tp.Generator[MergeContext, None, None]: ...
@tp.overload
@contextlib.contextmanager
def merge_context(
  ctxtag: tp.Hashable | None, inner: bool | None
) -> tp.Generator[MergeContext, None, None]: ...
@contextlib.contextmanager
