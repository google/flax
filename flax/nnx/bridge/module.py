...
from flax.nnx import graph, rnglib, traversals
...
def _module_meta_call(cls: type[M], *args, **kwargs) -> M:
  ...
  if parent_ctx is not None:
    if 'parent' in kwargs:
      ...
      state = getattr(parent, name, None)
  else:
    name = None
...
class Module(nnx_module.Module, ModuleBase, metaclass=ModuleMeta):
  ...
  def apply(
    ...
    **kwargs,
    ...
    states = ({},) if not variables else variables.values()
    graph.update(module, *states)
