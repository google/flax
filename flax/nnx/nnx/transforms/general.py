...
@tp.overload
def split_inputs(f: F, *, ctx_tag: str = 'split_merge_inputs') -> F: ...
def split_inputs(
  f: F | Missing = MISSING, *, ctx_tag: str = 'split_merge_inputs'
) -> F | tp.Callable[[F], F]:
...
def merge_inputs(
  ...
  *,
  ...
  def merge_inputs_wrapper(*args):
    ...
    if arg_states:
      graphdef = arg_states[0].graphdef
      states: graph.GraphState = State(
        {x.index: x.state.raw_mapping for x in arg_states}
      )
    ...
    (_, out), output_graph_nodes = extract.extract_graph_nodes(
      (inputs_graph_nodes, out)
    )

    graphdef_out, states_out = ctx.split(output_graph_nodes)

    def replace_index(x: extract.ExtractionIndex):
      return ArgState(
        x.index,
        graphdef_out,
    ...
    input_args_out = extract.replace_indexes(
      input_args, replace_index, clear=True
    )

    return input_args_out, out
