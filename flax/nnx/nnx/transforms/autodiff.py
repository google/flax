...
class GradFn:
  ...
  def __call__(self, *pure_args):
    ...
    def _grad_merge_fn(ctx: graph.MergeContext, path, value: extract.TreeNode):
  ...
  def grad_wrapper(*args, **kwargs):
    ...
    def _grad_split_fn(
      ...
      if prefix is None:
        ...
        graphdef, diff, nondiff = ctx.split(value, prefix.filter, ...)
...
def _custom_vjp_split_fn(
  ...
  *,
  ...
  if prefix is False:
    ...
    # but we reutrn a TreeNode.from_states which doesn't have a graphdef
    ...
    graphdef, passed, broadcast = ctx.split(value, prefix.filter, ...)
    nondiff_states.append(extract.GraphDefState(graphdef, broadcast))
    return extract.TreeNode.from_states(passed)
...
def custom_vjp(
  ...
  *,
  ...
  Example::
    ...
    ...   inputs_g, out_g = g
    ...   cos_x, sin_x, m = res
    ...   tangent_m = Foo(x=cos_x * out_g * m.y, y=sin_x * out_g)
    ...   return (tangent_m,)
    ...
    >>> f.defvjp(f_fwd, f_bwd)
    ...
