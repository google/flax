...
from flax.typing import MISSING, Leaf, Missing, PytreeDeque
import jax
...
class StateAxes:
  def __init__(
    self,
    filter_axes: tp.Mapping[filterlib.Filter, Index | type[Carry] | None]
...
def _scan_split_in(
  ...
  if graph.is_graph_node(x):
    ...
    if isinstance(prefix, StateAxes):
      raise ValueError(
        f'Cannot use StateAxes on non-graph nodes, '
        ...
        if axis is None:
          if is_input_arg:
            ...
            raise ValueError(
              ...
              f'Got filter {filter} and axis None at path {jax.tree_util.keystr(path)}'
            )
        elif isinstance(axis, int):
          vectorized_states.append(state)
        else:  # axis is Carry
          if is_input_arg:
            ...
            raise ValueError(
              ...
              f'Got filter {filter} and axis {axis} at path {jax.tree_util.keystr(path)}'
            )
      if is_input_arg:
        carry_deque.append(carry_states)
        broadcast_deque.append(broadcast_states)
...
class ScanFn:
  ...
  def __call__(
    ...
    pure_carry_arg_out, pure_scan_out = pure_out
    carry_out = (
      pure_carry_arg_out,
      carry_deque_out,
      broadcast_deque_out,
    ...
    return carry_out, (pure_args_out, pure_scan_out)
...
def scan(
  ...
  *,
  ...
  for key, x in jax.tree_util.tree_leaves_with_path(in_axes):
    ...
    if len(key) >= 2:
      raise ValueError(
        f'Carry must be used direcly on an input, it cannot be nested. '
        f'Found {in_axes=}'
      )
    if carry_argnum >= 0:
  ...
  def scan_wrapper(*args, **kwargs):
    ...
    carry_out, (pure_args_out, pure_scan_out) = jax.lax.scan(
      scan_fn,
      carry,
      pure_args,
      length=length,
