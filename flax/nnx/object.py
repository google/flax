...
class Object(reprlib.Representable, metaclass=ObjectMeta):
  ...
  def __init_subclass__(
    cls, pytree: tp.Literal['strict', 'auto'] = 'strict', **kwargs
  ) -> None:
    super().__init_subclass__(**kwargs)

    cls.__is_pytree = pytree
    ...
    if config.flax_mutable_array:
      if pytree == 'strict':
        ...
        jax.tree_util.register_pytree_with_keys(
          cls,
          flatten_with_keys=cls._pytree__auto_flatten_with_paths,
          unflatten_func=cls._pytree__auto_unflatten,
          flatten_func=cls._pytree__auto_flatten,
        )
      else:
        raise ValueError(
  ...
  def _setattr(self, name: str, value: tp.Any) -> None:
    ...
    if (
      ...
      and type(self).__is_pytree
      and name not in self._pytree__nodes
    ):
  ...
  def _pytree__auto_flatten_with_paths(self):
    graphdef, state = graph.split(self)
    key_values = sorted(
  ...
  def _pytree__auto_flatten(self):
    graphdef, state = graph.split(self)
    key_values = sorted(
      state.raw_mapping.items()  # type: ignore
    )
