...
def create_obj_env(object_types):
...
def tabulate(
  ...
  **input_kwargs,
  ...
  env = create_obj_env(object_types)

  # Modify all the object's methods to save their Tracer arguments.
  # tracer_args contains (object, name, args, kwargs) tuples.
  tracer_args: tuple[tp.Any, str, tuple, dict[str, tp.Any]] = []
  saver_env = {k: argsave(tracer_args, v) for k,v in env.items()}
  ...
  # Add VJP flops if required
  if compute_vjp_flops:
    for i, (object, name, args, kwargs) in enumerate(tracer_args):
      def do_vjp(*args, **kwargs):
        primals, f_vjp = _pure_nnx_vjp(jits[(type(object), name)], *args, **kwargs)
        return f_vjp(primals)
      rows[i].vjp_flops = _get_flops(jax.jit(do_vjp).lower(object, *args, **kwargs))
