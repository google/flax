...
def tabulate(
  ...
  **input_kwargs,
  ...
  variable_types = sorted(variable_types, key=lambda t: t.__name__)
  ...
  for row in rows:
    ...
    for var_type in variable_types:
      ...
      size_bytes = node_info.stats.get(var_type)
      if size_bytes:
...
def _unflatten_to_simple_structure(xs: list[tuple[tuple[tp.Any, ...], tp.Any]]):
  result = [] if len(xs) > 0 and isinstance(xs[0][0][0], int) else {}
