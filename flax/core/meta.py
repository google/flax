...
class AxisMetadata(metaclass=abc.ABCMeta):
  ...
  def replace_boxed(self, val: Any) -> TAxisMetadata:
    ...
    Args:
      value: The new value to be boxed by this AxisMetadata wrapper
    Returns:
...
def map_axis_meta(fn: Callable[[AxisMetadata], Any], tree: Any) -> Any:
  ...
  def wrapper(x):
    if isinstance(x, AxisMetadata):
      ...
      return x
  return jax.tree_map(wrapper, tree,
...
def replace_boxed(tree: Any, updates: Any) -> Any:
  ...
  def inner_update(c, v):
    if isinstance(c, AxisMetadata):
      ...
      return v
  return jax.tree_map(inner_update, tree, updates,
                      is_leaf=lambda x: isinstance(x, AxisMetadata))
...
class Partitioned(struct.PyTreeNode, AxisMetadata):
  ...
  Example::
    ...
    var_spec = nn.get_partition_spec(jax.eval_shape(mlp.init, random.PRNGKey(0), x))
  ...
  like ``nn.vmap`` and ``nn.scan``. In this case you can specifcy the name of
  ...
  def unbox(self, apply_constraint=True) -> Any:
    """Returns the wrapped value with the partioning applied as a sharding constraint."""
...
def get_partition_spec(tree: Any) -> Any:
  """Extracts a PartionSpec tree from a PyTree containing ``Partitioned`` values."""
  def f(x):
