...
def pack(
    ...
    def wrapper(scope_tree: Scope, *args):
        ...
        def scope_fn(variable_groups_xs_t, rng_groups_xs_t):
            ...
            for variable_groups, rng_groups, scope in zip(
                variable_groups_xs, rng_groups_xs, scopes
            ):
                variables = {}
                rngs = {}
...
def custom_vjp(
    ...
    def inner(scope_fn, repack_fn, variable_groups, rng_groups, *args):
        ...
        def f_bwd(*args):
            ...
            return backward_fn(
                *nondiff_args, simple_scope_fn, grad_variables, user_res, g_y
            )
...
def jit(
    ...
    def inner(scope_fn, repack_fn, variable_groups, rng_groups, *args):
        @functools.partial(
            jax.jit, static_argnums=static_argnums, device=device, backend=backend
        )
        @functools.wraps(fn)
