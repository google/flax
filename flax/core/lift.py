...
def pack(
  ...
  def wrapper(scope_tree: Scope, *args, **kwargs):
    ...
    for variable_group_xs in variable_groups_xs_t:
      for variable_group in variable_group_xs:
        for col_name, collection in variable_group.items():
          col_in_out = any(
              in_filter(col_filter, col_name) for col_filter in out_variable_filters
          )
          if not col_in_out:
    ...
    def scope_fn(
        variable_groups_xs_t, rng_groups_xs_t, mutable_filter: CollectionFilter = True
    ):
    ...
    try:
      if enable_kwargs:
        y, out_variable_groups_xs_t = fn(
            scope_fn, repack, variable_groups_xs_t, rng_groups_xs_t, *args, **kwargs
        )
      else:
        y, out_variable_groups_xs_t = fn(
...
def map_variables(
  ...
  return pack(
      wrapper, in_vars, out_vars, (rngs,), enable_kwargs=True, name='map_variables'
  )
...
def jvp(
  ...
  return pack(
      inner, (target, variables), (variables,), (rngs,), name='jvp', enable_kwargs=False
  )(scope, *primals)
...
def vmap(
  ...
  def inner(scope_fn, repack_fn, variable_groups, rng_groups, *args):
    ...
    for var_group, axis in zip(variable_groups, variable_in_axes):
      if axis is not None:
        new_variable_groups.append(meta.remove_axis(var_group, axis, metadata_params))
      else:
  ...
  return pack(inner, variable_in_groups, variable_out_groups, rng_groups, name='vmap')
...
def scan(
  ...
  rng_groups, rng_splits = _unzip2(split_rngs.items())
  rng_axes = tuple(0 if rng_split else axes_scan.broadcast for rng_split in rng_splits)
  ...
  def inner(scope_fn, repack_fn, variable_groups, rng_groups, init, *args):
    ...
    def scanned(broadcast_vars, carry, scan_variable_groups, rng_groups, args):
      ...
      if data_transform is not None:
        variable_groups, rng_groups = data_transform(variable_groups, rng_groups)
      scope = scope_fn(variable_groups, rng_groups)
      c, y = fn(scope, c, *args)
    ...
    broadcast_vars, (carry_vars, c), (ys, scan_vars) = scanned(
        broadcast_vars, (carry_vars, init), tuple(new_scan_vars), rng_groups, args
    )
    new_scan_vars = []
...
def while_loop(
  ...
  def inner(scope_fn, repack_fn, variable_groups, rng_groups):
    ...
    def body_wrapper(c):
      i, carry_variables, carry = c
      scope = scope_fn((carry_variables, broadcast_variables), make_loop_rngs(i))
...
def custom_vjp(
  ...
  rng_groups = (True,)
  return pack(
      inner, variable_in_groups, variable_out_groups, rng_groups, name='custom_vjp'
  )
...
def checkpoint(
  ...
  return pack(
      inner, (variables,), (variables,), (rngs,), name='remat', enable_kwargs=True
  )
