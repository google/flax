...
class Scope:
    ...
    def variable(
        self, col: str, name: str, init_fn: Callable[..., T], *init_args
    ) -> Variable[T]:
...
def apply(
    fn: Callable[..., Any], mutable: CollectionFilter = False
) -> Callable[..., Any]:
    ...
    def wrapper(
        ...
        if not _is_valid_variables(variables):
            raise ValueError(
                "The first argument passed to an apply function "
...
def init(
    fn: Callable[..., Any], mutable: CollectionFilter = True
) -> Callable[..., Any]:
