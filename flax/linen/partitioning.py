...
def logical_to_mesh_axes(array_dim_names,
  ...
  if isinstance(rules, (tuple, list)):
    result = [None] * len(array_dim_names)
    for rule_model_name, rule_mesh_name in rules:
      if rule_model_name in array_dim_names and rule_mesh_name not in result:
        if sum(rule_model_name == name for name in array_dim_names) > 1:
          raise ValueError(
              f'Unsupported: Dimension {rule_model_name} occurs more than once '
              'in array names.')
...
def _with_sharding_constraint(x, axis_resources):
...
def with_sharding_constraint(x, logical_axis_resources):
