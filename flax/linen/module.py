...
from flax.core.scope import Variable, VariableDict
...
def _module_repr(module: "Module", num_spaces: int = 4):
    ...
    rep = ""
    attributes = {
        k: v for k, v in cls.__annotations__.items() if k not in ("parent", "name")
    }
    child_modules = {
        k: v
...
class _DynamicContext:
    ...
    def module_stack(self):
        if not hasattr(self._thread_data, "module_stack"):
            self._thread_data.module_stack = [
                None,
            ]
...
# -----------------------------------------------------------------------------
def _get_suffix_value_pairs(tree_or_leaf: Any) -> List[Tuple[str, Type["Module"]]]:
...
_uninitialized_module_internal_state = _ModuleInternalState(False, False, None, None)
...
class Module:
    ...
    def _customized_dataclass_transform(cls):
        ...
        annotations["parent"] = Union[
            Type["Module"], Type["Scope"], Type["_Sentinel"], None
        ]
        ...
        dataclasses.dataclass(
            cls, unsafe_hash=True, repr=False
    ...
    def __setattr__(self, name: str, val: Any):
        ...
        # Modules have been passed in as dataclass args.
            ...
            name in self.__dataclass_fields__ and self.__dataclass_fields__[name].init
            ...
            for suffix, subvalue in _get_suffix_value_pairs(val):
                if isinstance(subvalue, Module):
                    ...
                    if subvalue.parent is _unspecified_parent:
                        ...
                        raise ValueError(
                            "Can't attach to remote parent in setup, pass in "
                ...
                # val is a parameter array or a Variable reference class.
                    ...
                    isinstance(
                        subvalue,
                        (np.ndarray, jax.interpreters.xla.DeviceArray, Variable),
                    )
                    and self._state.in_setup
                ):
                    var_name = f"{name}{suffix}"
                    # namecheck to ensure named variable matches self attribute name.
                    if (
                        suffix == ""
                        and self._state.last_varname  # not when assigning lists or dicts
                        and self._state.last_varname != var_name
                    ):
                        raise ValueError(
                            f"Variable name {self._state.last_varname} must "
    ...
    def _name_taken(self, name: str) -> bool:
        return name in self.scope.reservations or name in _all_names_on_object(self)
    ...
    def clone(
        self, *, parent: Optional[Union[Scope, "Module"]] = None, **updates
    ) -> "Module":
    ...
    def apply(
        ...
        **kwargs,
    ) -> Union[Any, Tuple[Any, VariableDict]]:
    ...
    def init_with_output(
        ...
        **kwargs,
        ...
        if not isinstance(rngs, dict):
            assert rngs.shape == (2,)
            rngs = {"params": rngs}
        return self.apply({}, *args, rngs=rngs, method=method, mutable=True, **kwargs)
