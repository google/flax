...
TestScope = type(
    'TestScope', (Scope,), {'make_rng': lambda self, name: jax.random.PRNGKey(0)}
)
...
def _get_unbound_fn(method_or_fn: Callable[..., Any]) -> Callable[..., Any]:
  ...
  """
  if inspect.ismethod(method_or_fn) and isinstance(method_or_fn.__self__, Module):  # pytype: disable=attribute-error
  ...
  if not callable(method_or_fn) or len(inspect.signature(method_or_fn).parameters) < 1:
...
class Module(ModuleBase):
  ...
  def _customized_dataclass_transform(cls, kw_only: bool):
    ...
    extra_fields = [
        ...
        ('name', Optional[str], kw_only_dataclasses.field(default=None, kw_only=True)),
    ]
    ...
    try:
      ...
      if add_call_info:
        ...
        _context.call_info_stack[-1].calls.append(
            _CallInfo(
                call_index, scope_path, type(self), fun.__name__, _args, _kwargs, _y
            )
        )
      return y
    finally:
  ...
  def __setattr__(self, name: str, val: Any):
    ...
    if not self._state.in_setup:
      if not self._state.is_initialized:
        ...
        raise errors.SetAttributeFrozenModuleError(self.__class__.__name__, name, val)
