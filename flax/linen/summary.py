...
class Row:
  ...
  Attributes:
    ...
    module_variables: Dictionary of variables in the module (no submodules included).
    counted_variables: Dictionary of variables that should be counted for this row,
      if no summarization is done (e.g. `depth=None` in `module_summary`) then this field
      is the same as `module_variables`, however if a summarization is done then this
      dictionary potentially contains parameters from submodules depending on the depth
  ...
  def size_and_bytes(self, collections: Iterable[str]) -> Dict[str, Tuple[int, int]]:
    return {
      ...
      if col in self.counted_variables else (0, 0)
...
class Table(List[Row]):
  ...
  * `collections`: a list containing the parameter collections (e.g. 'params', 'batch_stats', etc)
  """
  def __init__(self, module: 'flax.linen.Module', collections: List[str], rows: Iterable[Row]):
    super().__init__(rows)
    self.module = module
...
def tabulate(
  ...
  it returns a function of the form `(*args, **kwargs) -> str` where `*args` and `**kwargs`
  ...
  **Note**: rows order in the table does not represent execution order, 
  ...
  Returns:
    A function that accepts the same `*args` and `**kwargs` of the forward pass (`method`)
    and returns a string with a tabular representation of the Module's.
  """

  def _tabulate_fn(*args, **kwargs):
    table_fn = _get_module_table(module, rngs, method=method, mutable=mutable, 
...
def _get_module_table(
  ...
  def _get_table_fn(*args, **kwargs):
    ...
    shape_variables = jax.eval_shape(
        lambda: module.init(
          rngs,
          *args,
          method=method,
    ...
    collections: List[str] = [col for col in shape_variables.keys() if col != 'intermediates']
    shape_variables = shape_variables.unfreeze()
    rows = list(_flatten_to_rows(path=(), variables=shape_variables, depth=depth, output_methods=output_methods))
...
def _flatten_to_rows(
  ...
  if len(module_outputs) == 0:
    ...
    raise ValueError(f"""Cannot infer output, module '{'/'.join(path)}' has multiple intermediates: {list(module_outputs.keys())}""")
  else:
  ...
  if depth is not None and depth == 0:
    ...
    keys = list(
      key 
      for key in variables['intermediates'].keys() 
    ...
    for collection in variables:
      if collection != 'intermediates':
        for key in variables[collection]:
          if key not in keys and key not in module_variables.get(collection, {}):
            keys.append(key)
...
def _step_into(variables: Dict[str, Any], key: str):
  return {
    col: params[key]
    for col, params in variables.items()
    if key in params
  }
...
def _render_table(table: Table) -> str:
  ...
  for row in table:
    ...
    for collection, size_bytes in row.size_and_bytes(table.collections).items():
      ...
      if collection in row.module_variables:
        col_repr += _as_yaml_str(jax.tree_map(_format_value, row.module_variables[collection]))
        col_repr += '\n\n'
  ...
  rich_table.columns[1].footer = rich.text.Text.from_markup('Total', justify='right')
  ...
  for i, col in enumerate(table.collections):
    rich_table.columns[2 + i].footer = _size_and_bytes_repr(*collection_total[col])
