...
def in_q_fwd(compute_dtype, inp, scale, amax_history):
  qin, new_scale, new_history = q_and_return(
    inp, jnp.float8_e4m3fn, scale, amax_history, compute_dtype
  )
  return qin, (new_scale, new_history)


def in_q_bwd(compute_dtype, res, g):
  new_scale, new_history = res
  q_g = g
  return q_g, new_scale, new_history
...
def out_q_fwd(compute_dtype, out, scale, amax_history):
  return out, (scale, amax_history)
...
def out_q_bwd(compute_dtype, res, g):
  ...
  q_g, new_scale, new_history = q_and_return(
    g, jnp.float8_e4m3fn, scale, amax_history, compute_dtype #elfie investigate
  )
  return q_g, new_scale, new_history
...
def dot_general_with_precision(
  lhs, rhs, dimension_numbers, lhs_scale, rhs_scale, out_grad_scale, precision=None, preferred_element_type=None
):
  ...
  tmp = lax.dot_general(lhs, rhs, dimension_numbers, precision=lax.Precision.DEFAULT, preferred_element_type=preferred_element_type)
  tmp = dequantize(tmp, preferred_element_type, lhs_scale)
  tmp = dequantize(tmp, preferred_element_type, rhs_scale)
  return tmp


@dot_general_with_precision.defjvp
def dot_general_with_precision_jvp(
  dimension_numbers, lhs_scale, rhs_scale, out_grad_scale, precision, preferred_element_type, primals, tangents
):
  lhs, rhs = primals
  ...
  rhs = quantize(rhs, jnp.float8_e4m3fn, rhs_scale, preferred_element_type)
  out = lax.dot_general(
    lhs, rhs, dimension_numbers, precision=lax.Precision.DEFAULT, preferred_element_type=preferred_element_type
  )
  out = dequantize(out, preferred_element_type, lhs_scale)
  out = dequantize(out, preferred_element_type, rhs_scale)

  lhs_dot = dequantize(lhs_dot, preferred_element_type, out_grad_scale)
  lhs_dot = dequantize(lhs_dot, preferred_element_type, rhs_scale)
  part1 = lax.dot_general(
    lhs_dot, rhs, dimension_numbers, precision=lax.Precision.HIGHEST, preferred_element_type=preferred_element_type
  )

  rhs_dot = dequantize(rhs_dot, preferred_element_type, out_grad_scale)
  rhs_dot = dequantize(rhs_dot, preferred_element_type, lhs_scale)
  part2 = lax.dot_general(
    lhs, rhs_dot, dimension_numbers, precision=lax.Precision.HIGHEST, preferred_element_type=preferred_element_type
  )

  grad_out = part1 + part2
  return out, grad_out
...
class Fp8DotGeneralOp(module.Module):
  ...
  def __call__(self, *args, **kwargs):
    ...
    y_q = dot_general_with_precision(x, k, dimension_numbers, self.input_scale.value, self.kernel_scale.value, self.output_grad_scale.value,
                                     preferred_element_type=x.dtype)  # type: ignore
