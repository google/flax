...
print(jax.tree_util.tree_map(jnp.shape, nnx.state(m1)))

# lazy
m2 = Linear(dout=10)
y = m2.init(x=jnp.ones((1, 32)), rngs=nnx.Rngs(params=0))
print(jax.tree_util.tree_map(jnp.shape, nnx.state(m2)))
...
print(f'{y.shape=}')
print('state =', jax.tree_util.tree_map(jnp.shape, mnnx.state(lp)))
print()
