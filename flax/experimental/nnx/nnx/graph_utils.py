...
class GraphDef(tp.Generic[Node], reprlib.Representable):
  ...
  def __eq__(self, other):
    ...
    return (
      ...
      and self.idx_mapping == other.idx_mapping
    )
  ...
  def update(self, new_graphdef: GraphDef, state: State, /, *states: State):
    if self.flatten_ref_to_index is None:
      raise ValueError('Cannot update a graphdef without ref_to_index.')
    if new_graphdef.idx_mapping is None:
      raise ValueError('Cannot update a graphdef without idx_mapping.')

    if states:
      state = State.merge(state, *states)

    index_to_ref = compose_mapping_reversed(
      self.flatten_ref_to_index, new_graphdef.idx_mapping
    )

    graph_unflatten(self, state, ref_cache=index_to_ref)
...
def _graphdef_flatten(graphdef: GraphDef[Node]):
  ...
  static = (graphdef.nodedef, graphdef.idx_mapping)
  return (), static
...
def _graphdef_unflatten(
  ...
  nodedef, idx_mapping = static
  return GraphDef(nodedef, RefMap(), idx_mapping)
...
def graph_flatten(
  ...
  graphdef = GraphDef(nodedef, ref_to_index, None)
