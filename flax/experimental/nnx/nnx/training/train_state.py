...
class TrainState(nnx.Module):
  ...
  Example usage::
    ...
    >>> tx = optax.adam(1e-3)
    >>> state = nnx.train_state.TrainState(model, tx)
  ...
  Example usage::
    ...
    >>> class MetricsState(nnx.train_state.TrainState):
    ...
    >>> state = MetricsState(model, tx, metrics)
  ...
  """
  # TODO: running into an error when trying to call `.split` on TrainState
