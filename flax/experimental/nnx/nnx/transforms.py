...
def scan_apply(
  ...
  def scan_fn(
    ...
    (
      ...
      output_graph_nodes,
    ) = graph_utils.extract_graph_nodes((carry_arg_out, scan_args_out))
  ...
  carry_arg_out, scan_args_out = graph_utils.insert_graph_nodes(
...
def remat_apply(
  ...
  ctx = graph_utils.UpdateContext()
  args, input_nodes = graph_utils.extract_graph_nodes(args)
  graphdef, state = ctx.split(input_nodes)

  def _remat_fn(state: State, *args):
    input_nodes = ctx.merge(graphdef, state)
    args = graph_utils.insert_graph_nodes(args, input_nodes)
    out = f(*args)
