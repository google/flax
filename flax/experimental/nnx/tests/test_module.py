...
class TestModule:
  ...
  def test_update_static_state_submodules(self):
    ...
    m1 = Foo()
    refmap, graphdef, state = nnx.full_split(m1)
    m2, idxmap = nnx.full_merge(graphdef, state)
    m2.a.add_field()
    _, new_graphdef, state = nnx.full_split(m2, idxmap=idxmap)

    nnx.full_update(refmap, new_graphdef, state)
