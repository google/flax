...
class TestStochastic:
  ...
  def test_dropout_rng_override(self):
    ...
    y1 = m1(x)
    y2 = m2(x)
    np.testing.assert_raises(AssertionError, np.testing.assert_allclose, y1, y2)
  ...
  def test_dropout_arg_override(self):
    ...
    np.testing.assert_raises_regex(
      ValueError, 'No `deterministic` argument was provided to Dropout', m, x
    )
    ...
    np.testing.assert_raises(
      ...
      y,
      m(x, deterministic=False, rngs=nnx.Rngs(dropout=0)),
    )
    # no rng arg provided
    m.set_attributes(deterministic=False)
    np.testing.assert_raises_regex(
      ValueError,
      '`deterministic` is False, but no `rngs` argument was provided to Dropout',
      m,
      x,
    )
