...
class TestOptimizer(parameterized.TestCase):
  ...
  def test_jit(self, module_cls, jit_decorator, optimizer):
    ...
    if jit_decorator == jax.jit:
      model_state, model_static = state.model.split()
      loss_fn = lambda state, static, x, y: ((static.merge(state)(x)-y)**2).mean()
      initial_loss = loss_fn(model_state, model_static, x, y)

      def train_step(state, static, x, y):
        state = static.merge(state)
        model_state, model_static = state.model.split()
        grads = jax.grad(loss_fn)(model_state, model_static, x, y)
        state.apply_gradients(grads=grads)
        return state.split()

      state, static = jit_decorator(train_step)(*state.split(), x, y)
      state = static.merge(state)
