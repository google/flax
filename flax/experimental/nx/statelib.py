...
class TreeDef(tp.Generic[A]):
  ...
  path_index: tp.Mapping[PathParts, int]
...
def is_immutable_variable(path, x):
  return isinstance(x, Variable) and not x.mutable


def mutable(
  tree: A,
  /,
  do_mutate: tp.Callable[[PathParts, Variable], bool] = is_immutable_variable,
) -> A:
...
def is_mutable_variable(path, x):
  return isinstance(x, Variable) and x.mutable


def freeze(
  tree: A,
  /,
  do_freeze: tp.Callable[[str, Variable], bool] = is_mutable_variable,
) -> A:
...
def _update_recursive(current: State, updates: State, reuse_mutable: bool):
  for key, new_child_value in updates.items():
    if key not in current:
      raise ValueError(f'Path {key} not found in tree.')
    child_value = current[key]
    if isinstance(new_child_value, dict):
      if not isinstance(child_value, dict):
        raise ValueError(f'Expected dict at {key}, got {type(child_value)}.')
      _update_recursive(child_value, new_child_value, reuse_mutable)
    elif isinstance(child_value, dict):
      raise ValueError(f'Expected non-dict at {key}, got dict.')
    elif isinstance(child_value, Variable) and (
      isinstance(new_child_value, jax.Array)
    ):
      if reuse_mutable and child_value.mutable:
        child_value[...] = new_child_value[...]
      else:
        current[key] = child_value.copy(new_child_value[...])
    elif is_mutable_array(child_value) and (
      isinstance(new_child_value, jax.Array)
    ):
      if reuse_mutable and is_mutable_array(child_value):
        child_value[...] = new_child_value[...]
      else:
        current[key] = jax_core.mutable_array(new_child_value[...])
    else:
      current[key] = new_child_value
...
def always(path: PathParts, x):
  return True


def pure(
  tree: A, /, do_remove: tp.Callable[[PathParts, tp.Any], bool] = always
) -> A:
  def _remove_fn(path, x):
    path = _normalize_path(path)
    if isinstance(x, Variable) and do_remove(path, x):
      return x[...]
    return x
