...
class BatchNorm(base.Module):
    ...
    def apply(
        ...
        if self.is_stateful() or batch_stats:
            ...
            ra_var = self.state(
                "var", reduced_feature_shape, initializers.ones, collection=batch_stats
            )
        else:
            ra_mean = None
            ra_var = None
        ...
        if scale:
            mul = mul * self.param("scale", reduced_feature_shape, scale_init).reshape(
                feature_shape
            )
        y = y * mul
        if bias:
            y = y + self.param("bias", reduced_feature_shape, bias_init).reshape(
                feature_shape
            )
