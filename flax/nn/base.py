...
class _ModuleMeta(abc.ABCMeta):
    ...
    def __init__(cls, name, bases, attrs):
      ...
      """
            base_params = tuple(
                x
                for x in _fn_parameters(orig_fn)
...
class Module(metaclass=_ModuleMeta):
    ...
    def __new__(cls, *args, name=None, **kwargs):
        if not _module_stack:
            raise ValueError(
                "A Module should only be instantiated directly inside"
                " another module."
            )
        ...
        if parent.is_init and name not in parent.params:
            ...
            if name not in parent.params:
                raise ValueError(
                    f"No module named {name} was created during" " initialization."
                )
    ...
    def init(cls, _rng, *args, name=None, **kwargs):
        ...
        frame = _ModuleFrame(
            name, rng=_rng, parent=parent, transparent=cls._is_transparent()
        )
        with cls._with_instance(frame) as instance:
    ...
    def call(cls, params, *args, name=None, **kwargs):
        ...
        if name is None:
            name = cls._default_name()
        frame = _ModuleFrame(
            name, params=params, parent=parent, transparent=cls._is_transparent()
        )
        with cls._with_instance(frame) as instance:
            y = instance.apply(*args, **kwargs)
            _track_outputs(y)
    ...
    def _check_name(cls, name, parent):
        ...
        if name in parent.shared:
            ...
            if shared != parent.shared[name]:
                raise ValueError(
                    f'The name "{name}" is used for both a shared'
                    " and unshared module."
                )
...
def get_state():
    if not _state_stack:
        raise ValueError(
            "Use the flax.nn.stateful context manager to enable"
...
def _top_frame(call_name):
    if not _module_stack:
        raise ValueError(
            "%s should only be used inside a " "module's apply function." % call_name
        )
    return _module_stack[-1]
...
class Collection:
    ...
    def store(self, value):
        ...
        if not self._mutable:
            raise ValueError(
                "Collection is not mutable. Use the `mutate` method to"
        ...
        if value_level > self._master_level:
            raise ValueError(
                "Stateful operations are not allowed when the Collection"
        ...
        if frame == self._anchor:
            ...
            #   coll.store(1)
            raise ValueError(
                "State should be stored from within a module."
        ...
        if not frame.is_descendent_of(self._anchor):
            ...
            raise ValueError(
                "Trying to capture state outside the scope of this Collection."
        ...
        if self._root is None:
            ...
            if self._root.name is None or root.name is None:
                ...
                raise ValueError(
                    "When multiple top-level module calls use a Collection"
...
jax.tree_util.register_pytree_node(
    Collection, iterate_collection, collection_from_iterable
)


def _collection_state_dict(collection):
    return serialization._dict_state_dict(
        collection.as_dict()
    )  # pylint: disable=protected-access


def _collection_from_state_dict(xs, state):
    restored_state = serialization._restore_dict(
        xs.as_dict(), state
    )  # pylint: disable=protected-access
