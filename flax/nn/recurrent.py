...
from . import activation, base, initializers, linear
...
class LSTMCell(RNNCellBase):
    ...
    """
        ...
        dense_i = linear.Dense.partial(
            inputs=inputs, features=hidden_features, bias=False, kernel_init=kernel_init
        )
...
class OptimizedLSTMCell(RNNCellBase):
    ...
    class DummyDense(base.Module):
        ...
        def apply(self, inputs, features, kernel_init, bias_init, bias=True):
            k = self.param("kernel", (inputs.shape[-1], features), kernel_init)
            b = (
                self.param("bias", (features,), bias_init)
                if bias
                else jnp.zeros((features,))
            )
            return k, b
