...
class JaxToTfTestCase(absltest.TestCase):
  ...
  def assertAllClose(self, x, y, *, atol=None, rtol=None):
    """Asserts that x and y are close (up to numerical tolerances)."""
    kw = {}
    if atol:
      kw["atol"] = atol
    if rtol:
      kw["rtol"] = rtol
    assert_allclose = functools.partial(np.testing.assert_allclose, **kw)
    jax.tree_multimap(assert_allclose, x, y)
