...
def _make_mpa_dirs(mpa_targets: List[Tuple[MultiprocessArrayType, str]], tmp_path: str):
    ...
    for path in old_ckpts:
      if keep_every_n_steps:
        ...
        if step_number and (step_number - last_kept) >= keep_every_n_steps:
          logging.debug(
              'Not deleting %s, because last_kept=%f and keeping ' 'every %d steps.',
              path,
              last_kept,
              keep_every_n_steps,
          )
...
def _save_commit(
  ...
  if async_manager:
    jax.monitoring.record_event_duration_secs(
        '/jax/checkpoint/write/async/total_duration_secs', time.time() - ckpt_start_time
    )
...
def save_checkpoint(
  ...
  ckpt_path, ckpt_tmp_path, base_path = _get_checkpoint_paths(ckpt_dir, step, prefix)
  ...
  if config.flax_use_orbax_checkpointing or orbax_checkpointer:
    ...
    if orbax_checkpointer and isinstance(orbax_checkpointer, ocp.AsyncCheckpointer):
  ...
  if async_manager:
    ...
    save_main_ckpt_task()
  end_time = time.time()
  monitoring.record_event_duration_secs(_WRITE_CHECKPOINT_EVENT, end_time - start_time)
  return ckpt_path
  ...
  *  ``ckpt_-1.0, ckpt_1.0, ckpt_1e5 --> ckpt_1e5``
  ...
  if is_orbax:
    ...
    if target is not None:
      ...
      if isinstance(orbax_checkpointer._handler, ocp.PyTreeCheckpointHandler):  # pylint: disable=protected-access
        restore_kwargs['transforms'] = orbax_utils.maybe_construct_transformations(
  ...
  if _IMPORT_GDAM_SUCCESSFUL:
    state_dict = _restore_mpas(
        state_dict, target, ckpt_path, step, gda_manager, allow_partial_mpa_restoration
    )
