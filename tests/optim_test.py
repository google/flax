...
from flax import nn, optim, traverse_util
...
def check_eq(xs, ys, atol=None, rtol=None):
    ...
    assert jax.tree_util.tree_all(
        jax.tree_multimap(
            lambda x, y: onp.array(x).shape == onp.array(y).shape, xs_leaves, ys_leaves
        )
    ), "Leaves' shapes don't match."
    assert jax.tree_multimap(
        partial(_assert_numpy_allclose, atol=atol, rtol=rtol), xs_leaves, ys_leaves
    )
...
class OptimizerDefTest(absltest.TestCase):
    def test_create(self):
        ...
        expected_state = optim.OptimizerState(0, _MomentumParamState(onp.zeros((1,))))
    ...
    def test_compute_grad(self):
        ...
        self.assertEqual(grad, 3.0)
        self.assertEqual(aux, 4.0)

    def test_optimizer_with_focus(self):
        params = {"a": 0.0, "b": 0.0}
        opt_def = optim.GradientDescent(learning_rate=1.0)
...
class MultiOptimizerTest(absltest.TestCase):
    def test_multi_optimizer(self):
        params = {"a": 0.0, "b": 0.0, "c": {}}
        opt_a = optim.GradientDescent(learning_rate=1.0)
        opt_b = optim.GradientDescent(learning_rate=10.0)
        t_a = traverse_util.t_identity["a"]
        ...
        expected_hyper_params = [
            _GradientDescentHyperParams(1.0),
            _GradientDescentHyperParams(10.0),
        ]
        ...
        expected_params = {"a": 1.0, "b": 20.0, "c": {}}
        expected_state = [optim.OptimizerState(1, [()])] * 2
        self.assertEqual(new_state, expected_state)
        self.assertEqual(new_params, expected_params)
        # override learning_rate
        hp = optimizer_def.update_hyper_params(learning_rate=2.0)
        new_params, new_state = optimizer_def.apply_gradient(hp, params, state, grads)
        expected_params = {"a": 2.0, "b": 4.0, "c": {}}
        self.assertEqual(new_params, expected_params)
...
class MomentumTest(absltest.TestCase):
    ...
    def test_apply_gradient(self):
        optimizer_def = optim.Momentum(learning_rate=0.1, beta=0.2)
        params = onp.ones((1,))
        state = optim.OptimizerState(0, _MomentumParamState(onp.array([1.0])))
        grads = onp.array([3.0])
        new_params, new_state = optimizer_def.apply_gradient(
            optimizer_def.hyper_params, params, state, grads
        )
        ...
        expected_new_params = onp.array([1.0 - 0.32])
        self.assertEqual(new_params, expected_new_params)
        self.assertEqual(new_state, expected_new_state)
    ...
    def test_init_state(self):
        ...
        expected_state = optim.OptimizerState(
            ...
            _AdafactorParamState(
                onp.zeros((2,)), onp.zeros((3,)), onp.zeros((1,)), onp.zeros((1,))
            ),
        )
        ...
        expected_state = optim.OptimizerState(
            0,
            _AdafactorParamState(
                onp.zeros((1,)), onp.zeros((1,)), onp.zeros((3, 2)), onp.zeros((3, 2))
            ),
        )
        check_eq(state, expected_state)
...
class AdagradTest(absltest.TestCase):
    def test_init_state(self):
        ...
        self.assertEqual(optimizer_def.hyper_params, expected_hyper_params)
        expected_state = optim.OptimizerState(0, _AdagradParamState(onp.zeros((1,))))
        self.assertEqual(state, expected_state)

    def test_apply_gradient(self):
        optimizer_def = optim.Adagrad(learning_rate=0.1, eps=0.01)
        params = onp.array([1.0])
        state = optim.OptimizerState(1, _AdagradParamState(onp.array([0.1])))
        grads = onp.array([4.0])
        new_params, new_state = optimizer_def.apply_gradient(
            optimizer_def.hyper_params, params, state, grads
        )
...
class RMSPropTest(absltest.TestCase):
    def test_init_state(self):
        ...
        optimizer_def = optim.RMSProp(
            learning_rate=0.1, beta2=0.9, eps=0.01, centered=False
        )
        state = optimizer_def.init_state(params)
    ...
    def test_init_state_centered(self):
        params = onp.zeros((1,))
        optimizer_def = optim.RMSProp(
            learning_rate=0.1, beta2=0.9, eps=0.01, centered=True
        )
        state = optimizer_def.init_state(params)
    ...
    def test_apply_gradient(self):
        ...
        params = onp.array([1.0])
        state = optim.OptimizerState(1, _RMSPropParamState(onp.array([0.1]), None))
        grads = onp.array([4.0])
        new_params, new_state = optimizer_def.apply_gradient(
            optimizer_def.hyper_params, params, state, grads
        )
    ...
    def test_apply_gradient_centered(self):
        ...
        onp.testing.assert_allclose(
            new_state.param_states.v, expected_new_state.param_states.v
        )
        onp.testing.assert_allclose(
            new_state.param_states.mg, expected_new_state.param_states.mg
        )


class WeightNormTest(absltest.TestCase):
    def test_momentum_with_weight_norm(self):
        params = onp.ones((2, 2)) * 2.0
        optimizer_def = optim.WeightNorm(optim.Momentum(0.1))
        state = optimizer_def.init_state(params)
        self.assertEqual(
            jax.tree_map(onp.shape, state),
