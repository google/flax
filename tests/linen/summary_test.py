...
class SummaryTest(absltest.TestCase):
  ...
  def test_module_reuse(self):
    ...
    cnn = summary._get_module_table(CNN(), depth=1, show_repeated=True)(
        *args, mutable=True
    )

    block = summary._get_module_table(ConvBlock(), depth=1, show_repeated=True)(
        *args, mutable=True
    )
  ...
  def test_tabulate_param_count_and_flops(self):
    ...
    rep = module.tabulate(rng, x, console_kwargs=CONSOLE_TEST_KWARGS)
    lines = rep.splitlines()
    self.assertIn("Total Parameters: 50", lines[-2])

    table = summary._get_module_table(module, None, show_repeated=True)(
        rng, x, mutable=True
    )

    # Per-layer forward FLOPs.
    self.assertEqual(table[1].flops, 1216)  # 16 * 9 * 4 * 2 + 16 * 4
    self.assertEqual(table[2].flops, 288)  # 16 * 4 * 2 * 2 + 16 * 2

    # Total forward/backward FLOPs equal to their sums of per-layer FLOPs.
    self.assertEqual(table[0].flops, sum(r.flops for r in table[1:]))
    self.assertEqual(table[0].vjp_flops, sum(r.vjp_flops for r in table[1:]))

  def test_tabulate_non_diff(self):
    class Foo(nn.Module):

      @nn.compact
      def __call__(self, x):
        h = nn.Dense(2)(x)
        h = h.astype(jnp.bool_)
        return jax.lax.reduce(h, False, jax.lax.bitwise_xor, (1,))
    ...
    # Forward FLOPs.
    self.assertEqual(table[1].flops, 38)  # MLP: (9 * 2 * 2 + 2)
    self.assertEqual(table[0].flops, 41)  # Total: (9 * 2 * 2 + 2) + (2 + 1)

    # Backward FLOPs:
    self.assertEqual(table[1].vjp_flops, 130)  # (9 * 2 * 2 * 3 + 2) + 2 + 2 * 9
    self.assertEqual(table[0].vjp_flops, 0)  # Non-differentiable module
