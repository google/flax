...
class ModuleTest(absltest.TestCase):
    def test_init_module(self):
        ...
        onp.testing.assert_allclose(y, jnp.array([2.0]))
    ...
    def test_module_with_scope_is_not_hashable(self):
        ...
        with self.assertRaisesWithLiteralMatch(
            ValueError, "Can't call __hash__ on modules that hold variables."
        ):
            hash(module_a)
    ...
    def test_module_frozen(self):
        ...
        foo = Foo()
        with self.assertRaisesWithLiteralMatch(
            TypeError, "Module instance is frozen outside of setup method."
        ):
