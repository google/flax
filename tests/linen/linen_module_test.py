...
class ModuleTest(absltest.TestCase):
  ...
  def test_util_fun(self):
    ...
    self.assertEqual(
        param_shape, {'Dense_0': {'kernel': (10, 3)}, 'Dense_1': {'kernel': (3, 3)}}
    )
  ...
  def test_nested_module_reuse(self):
    ...
    self.assertEqual(
        param_shape,
        {'MLP_0': {'Dense_0': {'kernel': (10, 3)}, 'Dense_1': {'kernel': (3, 3)}}},
    )
  ...
  def test_setup_dict_nonstring_keys(self):
    ...
    param_shape = jax.tree_util.tree_map(jnp.shape, params)
    self.assertEqual(param_shape, {'a_(1, 2)': {'kernel': (3, 2), 'bias': (2,)}})
  ...
  def test_setattr_name_var_disagreement_allowed_in_lists(self):
    ...
    class Dummy(nn.Module):
      ...
      def setup(self):
        self.biases = [
            self.param(f'bias_{i}', initializers.ones, self.xshape) for i in range(4)
        ]
  ...
  def test_inheritance_dataclass_attribs(self):
    ...
    self.assertEqual(list(Test.__dataclass_fields__.keys()), ['bar', 'parent', 'name'])
    self.assertEqual(
        list(Test2.__dataclass_fields__.keys()), ['bar', 'baz', 'parent', 'name']
    )
    self.assertEqual(
        list(Test3.__dataclass_fields__.keys()), ['bar', 'baz', 'parent', 'name']
    )
    self.assertEqual(
        list(Test4.__dataclass_fields__.keys()), ['bar', 'baz', 'parent', 'name']
    )
  ...
  def test_get_suffix_value_pairs(self):
    ...
    self.assertEqual(
        nn.module._get_suffix_value_pairs({'a': 1, 'b': 2}), [('_a', 1), ('_b', 2)]
    )
    self.assertEqual(
        nn.module._get_suffix_value_pairs([1, 2, 3]), [('_0', 1), ('_1', 2), ('_2', 3)]
    )
    ...
    self.assertEqual(y2, [('_a', 1), ('_b_c', x2['b']['c']), ('_b_d', x2['b']['d'])])
  ...
  def test_jit_rng_equivalance(self):
    ...
    param_2 = jit_model.init(random.PRNGKey(0), np.ones((1, 1)))['params']['kernel']
