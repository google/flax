...
class PoolTest(parameterized.TestCase):
  ...
  @parameterized.parameters({'count_include_pad': True}, {'count_include_pad': False})
...
class NormalizationTest(parameterized.TestCase):
  ...
  def test_layer_norm(self, reduction_axes, use_fast_variance=True):
    ...
    y_one_liner = (x - x.mean(axis=reduction_axes, keepdims=True)) * jax.lax.rsqrt(
        x.var(axis=reduction_axes, keepdims=True) + e
    )
  ...
  def test_rms_norm(self, reduction_axes):
    ...
    e = 1e-5
    x = random.normal(key1, (2, 3, 4))
    model_cls = nn.RMSNorm(use_scale=False, epsilon=e, reduction_axes=reduction_axes)
    y, _ = model_cls.init_with_output(key2, x)
    self.assertEqual(x.dtype, y.dtype)
    self.assertEqual(x.shape, y.shape)
...
class StochasticTest(absltest.TestCase):
  ...
  def test_dropout_manual_rng(self):
    ...
    module = Foo()
    x1, x2 = module.apply({}, jnp.ones((20, 20)), rngs={'dropout': random.PRNGKey(0)})
