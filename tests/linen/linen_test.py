...
class RecurrentTest(absltest.TestCase):
  ...
  def test_lstm(self):
    ...
    lstm = nn.LSTMCell()
    carry, initial_params = lstm.init_with_output(
        key2, 1, 4, x, method=nn.LSTMCell.initialize_carry)
    self.assertEqual(carry.cell_state.shape, (2, 4))
    self.assertEqual(carry.hidden_state.shape, (2, 4))
    carry, y = lstm.apply(initial_params, carry, x)
    self.assertEqual(carry.cell_state.shape, (2, 4))
    self.assertEqual(carry.hidden_state.shape, (2, 4))
    np.testing.assert_allclose(y, carry.hidden_state)
  ...
  def test_gru(self):
    ...
    x = random.normal(key1, (2, 3))
    gru = nn.GRUCell()
    carry0, initial_params = gru.init_with_output(
        key2, 1, 4, x, method=nn.GRUCell.initialize_carry)
    self.assertEqual(carry0.shape, (2, 4))
    carry, y = gru.apply(initial_params, carry0, x)
  ...
  def test_convlstm(self):
    ...
    carry, initial_params = lstm.init_with_output(
        key2, 1, 6, x, method=nn.ConvLSTM.initialize_carry)
    self.assertEqual(carry.cell_state.shape, (2, 4, 4, 6))
    self.assertEqual(carry.hidden_state.shape, (2, 4, 4, 6))
    carry, y = lstm.apply(initial_params, carry, x)
    self.assertEqual(carry.cell_state.shape, (2, 4, 4, 6))
    self.assertEqual(carry.hidden_state.shape, (2, 4, 4, 6))
    np.testing.assert_allclose(y, carry.hidden_state)
  ...
  def test_optimized_lstm_cell_matches_regular(self):
    ...
    x = random.normal(key1, (2, 3))
    lstm = nn.LSTMCell()
    carry, lstm_params = lstm.init_with_output(
        key2, 1, 4, x, method=nn.LSTMCell.initialize_carry)
    self.assertEqual(carry.cell_state.shape, (2, 4))
    self.assertEqual(carry.hidden_state.shape, (2, 4))
    lstm = nn.LSTMCell()
    _, y = lstm.apply(lstm_params, carry, x)
    ...
    carry, lstm_opt_params = lstm_opt.init_with_output(
        key2, 1, 4, x, method=nn.OptimizedLSTMCell.initialize_carry)
    _, y_opt = lstm_opt.apply(lstm_opt_params, carry, x)
