...
class TensorboardTest(absltest.TestCase):
    ...
    def test_summarywriter_image(self):
        log_dir = tempfile.mkdtemp()
        summary_writer = SummaryWriter(log_dir=log_dir)
        expected_img = onp.random.uniform(low=0.0, high=255.0, size=(30, 30, 3))
    ...
    def test_summarywriter_image_float_pixel_values(self):
        log_dir = tempfile.mkdtemp()
        summary_writer = SummaryWriter(log_dir=log_dir)
        expected_img = onp.random.uniform(low=0.0, high=1.0, size=(30, 30, 3))
        summary_writer.image(tag="image_test", image=expected_img, step=1)
        summary_value = self.parse_and_return_summary_value(path=log_dir)

        # convert and scale expected_img appropriately to numpy uint8.
        expected_img = tf.image.convert_image_dtype(image=expected_img, dtype=onp.uint8)
    ...
    def test_summarywriter_2dimage_scaled(self):
        log_dir = tempfile.mkdtemp()
        summary_writer = SummaryWriter(log_dir=log_dir)
        img = onp.random.uniform(low=0.0, high=255.0, size=(30, 30))
    ...
    def test_summarywriter_single_channel_image_scaled(self):
        log_dir = tempfile.mkdtemp()
        summary_writer = SummaryWriter(log_dir=log_dir)
        img = onp.random.uniform(low=0.0, high=255.0, size=(30, 30, 1))
        img = img.astype(onp.uint8)
    ...
    def test_summarywriter_audio(self):
        ...
        self.assertTrue(
            onp.allclose(expected_audio_samples[0], actual_audio_1, atol=1e-04)
        )

        actual_audio_2 = tf.audio.decode_wav(summary_value.tensor.string_val[1]).audio
        self.assertTrue(
            onp.allclose(expected_audio_samples[1], actual_audio_2, atol=1e-04)
        )
