...
class TensorboardTest(absltest.TestCase):
  ...
  def test_summarywriter_scalar(self):
    ...
    self.assertTrue(
        np.allclose(tensor_util.make_ndarray(summary_value.tensor).item(), float_value)
    )
  ...
  def test_summarywriter_image_float_pixel_values(self):
    ...
    expected_img = tf.image.convert_image_dtype(image=expected_img, dtype=np.uint8)
  ...
  def test_summarywriter_audio(self):
    log_dir = tempfile.mkdtemp()
    summary_writer = SummaryWriter(log_dir=log_dir)
    expected_audio_samples = np.random.uniform(low=-1.0, high=1.0, size=(2, 48000, 2))
    summary_writer.audio(tag="audio_test", audiodata=expected_audio_samples, step=1)
    ...
    # Assert values.
    actual_audio_1 = tf.audio.decode_wav(summary_value.tensor.string_val[0]).audio
    self.assertTrue(np.allclose(expected_audio_samples[0], actual_audio_1, atol=1e-04))

    actual_audio_2 = tf.audio.decode_wav(summary_value.tensor.string_val[1]).audio
    self.assertTrue(np.allclose(expected_audio_samples[1], actual_audio_2, atol=1e-04))
  ...
  def test_summarywriter_audio_sampled_output(self):
    ...
    summary_writer.audio(
        tag="audio_test", audiodata=expected_audio_samples, step=1, max_outputs=1
    )
    ...
    self.assertTrue(np.allclose(expected_audio_samples[0], actual_audio, atol=1e-04))

  def test_summarywriter_clipped_audio(self):
    log_dir = tempfile.mkdtemp()
    summary_writer = SummaryWriter(log_dir=log_dir)
    expected_audio_samples = np.random.uniform(low=-2.0, high=2.0, size=(2, 48000, 2))
    summary_writer.audio(
        tag="audio_test", audiodata=expected_audio_samples, step=1, max_outputs=1
    )
    ...
    actual_audio = tf.audio.decode_wav(summary_value.tensor.string_val[0]).audio
    self.assertFalse(np.allclose(expected_audio_samples[0], actual_audio, atol=1e-04))
  ...
  def test_summarywriter_histogram_defaultbins(self):
    ...
    self.assertTrue(np.allclose(actual_histogram[0], (0.0, 33.3, 34.0), atol=1e-01))

  def test_summarywriter_histogram_2bins(self):
    log_dir = tempfile.mkdtemp()
    summary_writer = SummaryWriter(log_dir=log_dir)
    histogram = np.arange(1000)
    summary_writer.histogram(tag="histogram_test", values=histogram, step=1, bins=2)

    summary_value = self.parse_and_return_summary_value(path=log_dir)
    self.assertEqual(summary_value.tag, "histogram_test")
    actual_histogram = tensor_util.make_ndarray(summary_value.tensor)
    self.assertTrue(actual_histogram.shape, (2, 3))
    self.assertTrue(np.allclose(actual_histogram[0], (0.0, 499.5, 500.0), atol=1e-01))
    self.assertTrue(np.allclose(actual_histogram[1], (499.5, 999.0, 500.0), atol=1e-01))
