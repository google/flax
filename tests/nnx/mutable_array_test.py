...
class TestVariableRefMode(absltest.TestCase):
  ...
  def test_freeze_and_mutable(self):
    ...
    m3 = nnx.as_ref(m2)
    self.assertEqual(m3.a.mode, 'ref')
    self.assertIsNot(m2, m3)
    self.assertIsNot(m2.a, m3.a)

  @nnx.variable_mode('lojax')
  def test_to_arrays_example(self):
    ...
    shared_array = nnx.Variable(1.0, mode='lojax')
  ...
  def test_freeze_and_mutable_with_filter(self):
    ...
    m = Foo()
    self.assertEqual(m.a.mode, 'ref')
    self.assertEqual(m.b.mode, 'ref')

    m2 = nnx.as_lojax(m, only=nnx.BatchStat)
    self.assertEqual(m2.a.mode, 'ref')
    self.assertEqual(m2.b.mode, 'lojax')
    self.assertIsNot(m, m2)

    m3 = nnx.as_ref(m2, only=nnx.BatchStat)
    self.assertEqual(m3.a.mode, 'ref')
    self.assertEqual(m3.b.mode, 'ref')
    self.assertIsNot(m2, m3)
  ...
  def test_mutable_array_split_merge_in_variable_shared_array(self):
    class Foo(nnx.Module):
      def __init__(self):
        m_array = 1
        self.a = nnx.Param(m_array, mode='ref')
        self.b = nnx.Param(m_array, mode='ref')
  ...
  def test_mutable_array_split_freeze(self):
    ...
    ref_map = nnx.graph.RefMap()
    graphdef, state = nnx.graph.flatten(m, ref_index=ref_map)
    state = nnx.as_lojax(state)
    self.assertLen(state, 1)

    m1 = nnx.merge(graphdef, nnx.as_hijax(state))
    self.assertIs(m1.a, m1.b)
    self.assertIsInstance(m1.a, jax.Ref)
...
class TestOptimizer(absltest.TestCase):
  ...
  def test_optimize_mutable_arrays(self):
    ...
    def train_step(model, optimizer, x, y):
      ...
      loss, grads = jax.value_and_grad(loss_fn)(nnx.as_lojax(params))
...
class TestHijaxVariables(absltest.TestCase):
  def test_variable_to_hijax(self):
    ...
    v_hi = nnx.as_hijax(v_low)

    self.assertTrue(v_hi.mode == 'hijax')
    ...
    v_low = nnx.as_lojax(v_hi)
    self.assertTrue(v_low.mode == 'lojax')
  ...
  def test_use_hijax(self):
    ...
    self.assertEqual(v_hi.mode == 'hijax', True)

    with nnx.variable_mode('hijax'):
      v2 = nnx.Param(1, a='hi')
      self.assertEqual(v2.mode == 'hijax', True)
  ...
  def test_return_hijax_from_transform(self):
    ...
    v = create_var()
    self.assertEqual(v.mode == 'hijax', True)
