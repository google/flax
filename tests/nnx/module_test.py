...
class TestCapture(parameterized.TestCase):
  ...
  def test_scan_with_list_subclass_capture(self):
    class CapturingList(nnx.List):
      def __call__(self, x):
        self.sow(nnx.Intermediate, 'input', x)
        for layer in self:
          x = layer(x)
        return x
    ...
    self.assertEqual(intermediates['layers']['input'][0].shape, (3, 4))
    self.assertFalse(hasattr(model.layers, '__captures__'))
