...
class TestSPMD(absltest.TestCase):
  ...
  def test_init(self):
    ...
    assert m.w.sharding.shard_shape(m.w.value.shape) == (4, 1)
    assert m.w.sharding.is_equivalent_to(
      NamedSharding(mesh, P('model', 'data')), ndim=2)
    assert y.sharding.is_equivalent_to(
      NamedSharding(mesh, P(None, 'data')), ndim=2)
  ...
  def test_logical_rules(self):
    ...
    with set_mesh(mesh), nnx.logical_axis_rules((('col-alias', 'col'),)):
      model = Foo()
  ...
  def test_annotate_axis_in_transform(self):
    class Foo(nnx.Module):
      def __init__(self, rngs):
        @nnx.set_transform_axis_name(name='layers')
        @partial(jax.vmap, spmd_axis_name='layers')
        def create_sublayers(r):
          return nnx.Linear(8, 8, rngs=r, use_bias=False,
                            kernel_init=nnx.with_partitioning(
                              nnx.initializers.lecun_normal(), (None, 'model')))
        self.layers = create_sublayers(rngs.fork(split=4))

      def __call__(self, x):
        @nnx.set_transform_axis_name(name='layers')
        def scan_over_layers(x, layer):
          assert layer.kernel.shape == (8, 8)
          assert layer.kernel.sharding_names == (None, 'model')
          return layer(x), None
        x, _ = jax.lax.scan(scan_over_layers, x, self.layers)
        return x

    mesh = jax.make_mesh((1, 2, 2), ('layers', 'batch', 'model'))
    with set_mesh(mesh):
      model = Foo(nnx.Rngs(0))
      x = jax.device_put(jnp.zeros((4, 8)), P('batch', None))
      y = model(x)
    assert model.layers.kernel.sharding_names == ('layers', None, 'model')
    assert model.layers.kernel.sharding.is_equivalent_to(
      NamedSharding(mesh, P('layers', None, 'model')), ndim=3)
    assert y.sharding.is_equivalent_to(
      NamedSharding(mesh, P('batch', 'model')), ndim=2)
