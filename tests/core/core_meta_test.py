...
class MetaTest(absltest.TestCase):
  ...
  def test_boxed_param(self):
    def f(scope, xs):
      def g(scope, x):
        kernel_init = meta.with_partitioning(nn.initializers.ones_init(), ('in', 'out'))
    ...
    self.assertEqual(variables['params']['kernel'].names, ('batch', 'in', 'out'))
    ...
    def f(scope, xs):
      def g(scope, x):
        ...
        kernel = scope.variable(
            'params', 'kernel', kernel_init, scope.make_rng('params'), (x.shape[-1], 2)
        )
        kernel.value += 1.0
