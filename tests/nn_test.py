...
class DataDependentInitModule(nn.Module):
    def apply(self, x):
        bias = self.param("bias", x.shape, lambda rng, shape: x + 1.0)
        return x + bias
...
class ModuleTest(absltest.TestCase):
    ...
    def test_init_by_shape_module(self):
        ...
        self.assertEqual(y2, jnp.array([2.0]))
        self.assertEqual(params, {"bias": jnp.array([1.0])})
    ...
    def test_shared_module(self):
        ...
        y = model(x)
        self.assertEqual(y, jnp.array([3.0]))
        self.assertEqual(model.params, {"dummy": {"bias": jnp.array([1.0])}})
    ...
    def test_module_state(self):
        class StatefulModule(nn.Module):
            def apply(self, x, coll=None):
                state = self.state(
                    "state", x.shape, nn.initializers.zeros, collection=coll
                )
                state.value += x
    ...
    def test_parameter_rng(self):
        ...
        def model(x):
            return nn.Dense(
                x, features=2, name="dummy", bias_init=nn.initializers.normal()
            )
...
class CollectionTest(absltest.TestCase):
    ...
    def test_collection_multiple_roots(self):
        ...
        expected_state = {
            "/a/dummy": jnp.array([3.0]),
            "/b/dummy": jnp.array([3.0]),
        }
        self.assertEqual(activations.state, expected_state)
        with self.assertRaises(ValueError):
            with nn.Collection().mutate() as activations:
...
class PoolTest(absltest.TestCase):
    def test_pool_custom_reduce(self):
        ...
        onp.testing.assert_allclose(y, onp.full((1, 2, 2, 1), 2.0 ** 4))

    def test_avg_pool(self):
        x = jnp.full((1, 3, 3, 1), 2.0)
        pool = lambda x: nn.avg_pool(x, (2, 2))
        y = pool(x)
        onp.testing.assert_allclose(y, onp.full((1, 2, 2, 1), 2.0))
...
class NormalizationTest(absltest.TestCase):
    def test_batch_norm(self):
        ...
        mean = y.mean((0, 1))
        var = y.var((0, 1))
        onp.testing.assert_allclose(mean, onp.array([0.0, 0.0]), atol=1e-4)
        onp.testing.assert_allclose(var, onp.array([1.0, 1.0]), rtol=1e-4)
    ...
    def test_layer_norm(self):
        ...
        y_one_liner = (x - x.mean(axis=-1, keepdims=True)) * jax.lax.rsqrt(
            x.var(axis=-1, keepdims=True) + e
        )
    ...
    def test_group_norm(self):
        ...
        y, _ = nn.GroupNorm.init(
            key2, x, num_groups=2, bias=True, scale=True, epsilon=e
        )
        self.assertEqual(x.shape, y.shape)
        self.assertIsInstance(y, type(x))

        x_gr = x.reshape([2, 5, 4, 4, 2, 16])
        y_test = (x_gr - x_gr.mean(axis=[1, 2, 3, 5], keepdims=True)) * jax.lax.rsqrt(
            x_gr.var(axis=[1, 2, 3, 5], keepdims=True) + e
        )
...
class RecurrentTest(absltest.TestCase):
    ...
    def test_optimized_lstm_cell_matches_regular(self):
        ...
        lstm_opt = nn.Model(
            nn.OptimizedLSTMCell.partial(name="LSTMCell"), initial_params
        )
