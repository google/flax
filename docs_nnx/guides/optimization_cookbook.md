...
This notebook goes through some common problems in nontrivial training loops for Flax models. For clarity, all sections below will be training the following toy model.
...
def make_model(rngs, **metadata):
    return nnx.Sequential(
        nnx.Linear(2,8, rngs=rngs, kernel_metadata=metadata),
        nnx.Linear(8,8, rngs=rngs, kernel_metadata=metadata))
...
But we don't need to replicate the optimizer state, as it's not invovled in SPMD computations. One copy is enough, and we can shard this copy across our mesh to reduce memory usage. This means that we need the optimizer state to be sharded differently from the parameters themselves. To do this, we can pass 'opt_sharding' as kernel metadata when initializing our layers. 
...
model = make_model(rngs, opt_sharding=P('x', 'y'))
