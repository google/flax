...
We saw above how to use `rng.split` when passing explicit random state through [Flax NNX transforms](https://flax.readthedocs.io/en/latest/guides/transforms.html) like `nnx.vmap` or `nnx.pmap`. We can do the same for implicit random state using the [View](https://flax.readthedocs.io/en/latest/guides/view.html) `nnx.with_rngs`. Consider the example below, which generates a batch of samples from the nondeterministic model we defined above.

```python
in_axes = nnx.prefix(model, {'dropout': 0, ...: None}, graph=False)

@nnx.with_rngs(split=5, only='dropout', graph=False)
@nnx.vmap(in_axes=(in_axes, None), out_axes=0, graph=False)
...
The call to the decorator `with_rngs` with the `split` argument makes `sample_from_model` split the rngs of the `model` argument before the function body is applied. 
Within the function body, we tell `vmap` to only map over values within Variables tagged as "dropout" using `nnx.prefix` to specify the `in_axes`.
...
- The key ingredient of recurrent dropout is to apply the same dropout mask across all time steps. To achieve this, you must use `nnx.with_rngs` with the `broadcast` argument. This will broadcast the `recurrent_dropout` stream within the implicit state. To specify that this state should traversed in the call to `nnx.scan`, you can use a call to `nnx.prefix` to generate the appropriate `in_axes` argument. 
- The hidden state `h` will be the `nnx.scan`'s `Carry` variable, and the sequence `x` will be `scan`ned over its axis `1`.
...
def rnn_forward(cell: RNNCell, x: jax.Array):
    ...
    broadcast_cell = nnx.with_rngs(cell, broadcast=x.shape[1], graph=False)
    in_axes = nnx.prefix(broadcast_cell, {'recurrent_dropout': 0, ...: None}, graph=False)
    @nnx.scan(in_axes=(in_axes, nnx.Carry, 1), out_axes=(nnx.Carry, 1), graph=False)
    def unroll(cell, h, x) -> tuple[jax.Array, jax.Array]:
        h, y = cell(h, x)
        return h, y
    _, y = unroll(broadcast_cell, h, x)
    return y
